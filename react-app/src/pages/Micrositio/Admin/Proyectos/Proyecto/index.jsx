...
export default function ProyectoAdmin() {
  ...
  const [newEnlaces, setNewEnlaces] = useState([]);
  ...
  const onSubmit = (data) => {
    ...
    console.log(JSON.stringify(newEnlaces));
  ...
  const updateEnlaces = (index,field,value) => {
    const newEnlacesCopy = [...newEnlaces];
    newEnlacesCopy[index][field] = value;
    setNewEnlaces(newEnlacesCopy);
  }
        ...
        <form id="formUpdateProyecto" onSubmit={handleSubmit(onSubmit)} className="w-full flex">
          ...
          <div className="pl-5 mt-6 w-[30%]">
            <div className="border border-secondary w-full py-5 flex flex-col justify-between items-center bg-[#FBF9F6] rounded-md shadow-header">
               ...
               {
                newSecciones.map((seccion,index)=>{
                    return(
                      <div className="mt-6 bg-white py-6 px-10 rounded-lg shadow-header">
                        ...
                        <button
                          onClick={()=>{
                            setNewEnlaces([...newEnlaces,{titulo:"",url:""}])
                          }}
               ...
               {
                newEnlaces.map((enlace,index)=>{
                    return(
                        <div key={index} className="flex flex-col mt-4">
                            <input onChange={(e)=>updateEnlaces(index,"titulo",e.target.value)} className="border-b border-b-transparent hover:border-b-secondary font-bold" placeholder="Titulo del enlace"/>
                            <input onChange={(e)=>updateEnlaces(index,"url",e.target.value)} className="font-light border-b border-b-transparent hover:border-b-secondary text-primary" placeholder="URL del enlace"/>
                        </div>
                    )
                })
               }
            <button
            onClick={()=>{
                setNewSecciones([...newSecciones ,{titulo:""}])
            }}
            type="button" className="w-full text-secondary mt-3 px-3 py-2 hover:bg-secondary/10 rounded-md">
            + Agrega una sección 
          </button>

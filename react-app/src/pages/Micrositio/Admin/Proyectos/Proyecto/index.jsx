      ...
      <Main>
        ...
        <form id="formUpdateProyecto" onSubmit={handleSubmit(onSubmit)} className="w-full flex">
          ...
          <div className="pl-5 mt-6 w-[30%]">
            ...
            {
              newSecciones.length > 0 &&
            <div className="mt-6 bg-white py-6 px-10 rounded-lg shadow-header">
               {
                newSecciones.map((seccion,index_seccion)=>{
                    return(
                      <span key={index_seccion}>
                        <input value={seccion.titulo} onChange={(e)=>{updateNewSecciones(index_seccion, "titulo", e.target.value)}} className="border-b border-secondary font-timesnr text-secondary text-xl mb-5 w-full" placeholder="Escribe el titulo de la sección"></input>
                        {

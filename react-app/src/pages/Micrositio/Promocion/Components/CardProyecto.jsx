...
export default function CardProyecto({ proyecto }) {
  return (
    <div className="flex bg-white shadow-header w-full h-56 justify-between rounded-lg">
        <div className="flex flex-col h-full justify-between p-5 w-2/3">
            <p className="font-timesnr text-3xl">{proyecto.nombre}</p>
            <p className="font-light text-sm">{proyecto.descripcion}</p>
            <Link className="flex px-3 py-1.5 bg-primary rounded-3xl text-white font-medium w-24 items-center" to={"/micrositio/proyectos/"+proyecto.id}>

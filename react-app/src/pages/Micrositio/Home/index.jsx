...
export default function HomeMicrosito() {
  ...
  const filterPersonas = (persona) => {
    return (
      ...
      (filteredPuestos.length === 0 ||
        ...
        && 
      (filteredPuestosPrograma.length === 0 || puestosInterceptionNotZero(filteredPuestosPrograma, persona?.puesto_programas))
    );
  };
  ...
  const map_personas_to_report = (personas) => {
    ...
    const transformed_personas = persona_copy?.map((persona) => {
      ...
      return { ...persona, puestos_programa: reduced_puestos_programa, puestos_escuela: reduced_puestos };
    });

          ...
          <div className="flex flex-wrap md:ml-96 w-full">
            {
              horarios.filter(filtrar).map((horario) => (
                <div key = {horario.id_horario} onClick={()=>{navigate("/horario",{state:{id_horario: horario.id_horario }})}} className="rounded-3xl bg-primarylight w-80 ml-9 mb-4 p-2.5 md:ml-2 min-h-28">
                  <div>
                    <p className="text-sm font-poppins text-primary font-semibold mb-2">Clase {horario.no_clase}</p>
                  </div>
                  <p className="font-poppins text-gray1 text-sm mb-1">{horario.salon} • {horario.dia}, {horario.hora_servicio_inicio?horario.hora_servicio_inicio.substring(0,5):horario.hora_inicio.substring(0,5)} a {horario.hora_servicio_fin?horario.hora_servicio_fin.substring(0,5):horario.hora_fin.substring(0,5)}</p>

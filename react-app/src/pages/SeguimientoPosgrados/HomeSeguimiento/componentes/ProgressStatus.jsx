export default function ProgressStatus(props) {
  if (Math.trunc(props.porcentaje) >= 99 && props.fecha_proxima_actualizacion) {
    return (
      ...
      <div className="w-full h-full py-2 relative text-black flex items-center justify-center">
        ...
        >
          ...
          <circle
            ...
            fill="none"
          ></circle>
          <text x={`${props.porcentaje <= 99 ? "7" : "5"}`} y="15" fontSize="7">
            {props.porcentaje >= 99 ? 100 : Math.trunc(props.porcentaje)}%
          </text>
        </svg>
      </div>
    );
